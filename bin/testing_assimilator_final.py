...
def DR3_replacement(data):
    ...
    for patient in ['Recip']:
        for variable in ['First', 'Second']:
            ...
            for row in range(rows):
                DR_column = "HR_" + patient + '_' + variable + '_DR_Split'
                dr = data.loc[row, DR_column]

                if pd.isnull(data.loc[row, DR_column]):
                    pass
                elif dr == 'DR3':
                    data.loc[row, DR_column] = re.sub('DR3','DR17', dr)
...
def cw7_special_cases(data):
    ...
    for patient in ['Recip']:
        for variable in ['First', 'Second']:
            ...
            for row in range(rows):
                c_col = "HR_" + patient + '_' + variable + '_C_Split'
                c_col_alt = "HR_" + patient + '_' + variable2 + '_C_Split'
                b1_col = "HR_" + patient + '_First_B_Split'
                b2_col = "HR_" + patient + '_Second_B_Split'
                c = data.loc[row, c_col]
                c_alt = data.loc[row,c_col_alt]
                b1 = data.iloc[row][b1_col]
                b2 = data.iloc[row][b2_col]
                ...
                elif c == 'Cw7' and c_alt == 'Cw7':
                    c1 = "HR_" + patient + '_First_C_Split'
                    c2 = "HR_" + patient + '_Second_C_Split'
                    if ((b1 == "B*07:02") and (b2 == "B*08:01")):
                        data.loc[row, c1] = re.sub('Cw7', 'C*07:02', c)
                        data.loc[row, c2] = re.sub('Cw7', 'C*07:01', c)
                    elif ((b1 == "B*08:01") and (b2 == "B*07:02")):
                        data.loc[row, c2] = re.sub('Cw7', 'C*07:02', c)
                        data.loc[row, c1] = re.sub('Cw7', 'C*07:01', c)
                ...
                elif c == 'Cw7' and c_alt == 'C*15:02':
                    if ((b1 == "B*07:02") and (b2 == "B*08:01")):
                        data.loc[row, c_col] = re.sub('Cw7', 'C*07:01', c)
                    elif ((b1 == "B*08:01") and (b2 == "B*07:02")):
                        data.loc[row, c_col] = re.sub('Cw7', 'C*07:01', c)
                    elif ((b1 in C0702_options) or (b2 in C0702_options)):
                        data.loc[row, c_col] = re.sub('Cw7', 'C*07:02', c)
                    else:
                        data.loc[row, c_col] = re.sub('Cw7', 'C*07:01', c)
                elif c == 'Cw7' and ((b1 in C0702_options) or (b2 in C0702_options)):
...
def assign_sub_codes(data,options,log_path,patient,classII):
    ...
    with open(logfilename, "w+") as log_file:
        ...
        for row in range(rows):
            R1 = str(data.loc[row, ("HR_" + patient + "_First_DR_Split")]).strip()
            R2 = str(data.loc[row, "HR_" + patient + "_Second_DR_Split"]).strip()
            Q1 = str(data.loc[row, "HR_" + patient + "_First_DQ_Split"]).strip()
            Q2 = str(data.loc[row, "HR_" + patient + "_Second_DQ_Split"]).strip()
